document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchTramStops() {
      const tramStops = await fetch('/route_11_outbound_stops.json').then(response => response.json());
      return tramStops;
    }
    ...
    async function renderGigs(gigs, stops) {
      ...
      for (const gig of gigs) {
        ...
        const travelTime = await fetchTravelTime(nextTram.runId, startStopId, closestStop.stop_id);
