...
function appendGigList(gigs, gigList, category, stops, nextTramData, venueArrivalTimes, venueStopMapping, currentStop) {
    ...
    gigs.forEach((gig) => {
        ...
        // Retrieve venueStopId from the mapping
        const venueStopId = venueStopMapping[gig.venue.id]; 

        // Check if venueStopId is valid before proceeding
        if (!venueStopId) {
            console.warn(`No venue stop ID found for gig: ${gig.name}, venue ID: ${gig.venue.id}`);
            return;
        }

        // Find the venue stop in the stops array
        const venueStop = stops.find(stop => stop.stop_id === venueStopId);
        if (!venueStop) {
            console.warn(`No stop found with stop ID: ${venueStopId} for venue: ${gig.venue.name}`);
            return;
        }
